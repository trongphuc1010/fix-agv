...
const float Ki = 0.01;
const float TIME_VAO_CUA = 200.0; // T/g hạ tốc độ và bẻ lái gắt dần khi xe lệch ra ngoài
const float TIME_RA_CUA  = 500.0; // T/g đề ba và nhả thẳng lái dần khi xe về lại vạch tâm
...
void TinhToanVaXuatPID() 
{
    ...
    // 1. Xác định CÁC THÔNG SỐ ĐÍCH (Target) dựa trên nhóm Zone 0, 123, 456, 789
    ...
    // Cân đối target offset theo tốc độ thực (như code gốc)
    ...
    // 3. TÍNH TOÁN XUẤT XUNG VỚI THÔNG SỐ ĐÃ ĐƯỢC LÀM MƯỢT
    int finalBaseSpeed = baseSpeed + (int)current_offset; // Hạ tốc độ / Đề ba từ từ
    ...
    // PID tính toán dựa trên current_Kp và current_Kd (đang trượt) thay vì số cứng
