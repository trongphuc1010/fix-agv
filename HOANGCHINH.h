...
const unsigned long TIME_DELAY_CASE_1 = 200; //(0 SANG 1 / -1)
...
// =========================================================================
// TRẢ LẠI HÀM ĐỌC CẢM BIẾN GỐC 100% CỦA BẠN (SẠCH SẼ, KHÔNG LỖI)
// =========================================================================
...
void TinhToanVaXuatPID() 
{
    ...
    // =========================================================================
    // AN TOÀN: BỘ ĐẾM GIỜ CASE 1 ĐƯỢC CHUYỂN XUỐNG ĐÂY
    // (Chỉ thao tác trên biến ảo absError, không làm hỏng mắt quét gốc)
    // =========================================================================
    ...
    if (dang_cho_case_1) {
        if (rawAbsError == 1) {
            // Đang bị giữ lại bởi timer -> Coi như chưa lệch (ép = 0)
            if (millis() - time_cho_case_1 < TIME_DELAY_CASE_1) {
                effective_absError = 0; 
            } else {
                dang_cho_case_1 = false;
            }
        } else {
            dang_cho_case_1 = false; // Văng ra xa hơn hoặc về 0 -> Tắt bộ đếm
        }
    }
    ...
    // Cập nhật lại sai số PID theo giá trị đã qua bộ lọc
    float active_error = (steering_error > 0) ? absError : -absError;
    if (steering_error == 0) active_error = 0; // Tránh lỗi dấu
